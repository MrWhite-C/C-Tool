...
def copy_os():
    os.system("cd && cd ../.. && echo \"即将开始备份\" && tar -cvzf Termux系统备份.tar.gz files/ && mv Termux系统备份.tar.gz /sdcard/C-Tool/OS")
    print(TGreen+"""
    如果成功那么在/sdcard/C-Tool/OS会有一个叫Termux系统备份.tar.gz的东东
...
#Shell Scripts
installjava="wget https://github.com/Hax4us/java/archive/master.zip && unzip master.zip && cd java-master && sh installjava && cd ../ && rm -rf java-master"
downloadall="apt upgrade && apt update "
download_annie="cd && wget https://github.com/iawia002/annie/releases/download/0.10.3/annie_0.10.3_Linux_ARM64.tar.gz && tar -xvzf annie_0.10.3_Linux_ARM64.tar.gz && chmod +x annie && mv annie ../usr/bin && rm annie_0.10.3_Linux_ARM64.tar.gz"
...
\____/    /_/  \____/\____/_/"""+BRed+"BetaV0.14.4"+reset_colors
...
def program_enviroment():
    ...
    choose=input(TGreen+"C-Tool User => "+reset_colors)
    if choose == '1':
        print(BGreen+"如果是wget:not found请输入："+reset_colors)
        print(TGreen+"pkg install wget -y"+reset_colors)
        time.sleep(3)
        os.system("clear")
        print(BSkyblue+"即将开始脚本"+reset_colors)
...
def useful_tools():
    ...
    if choose == '1':
        print(BRed+"""
请确保您安装了wget
用apt-get install wget
或pkg install wget"""+reset_colors)
        sleep(2)
